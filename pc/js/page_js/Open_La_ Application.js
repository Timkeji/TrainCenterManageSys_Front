...
$(document).ready(function (){
    $.get(SERVER_PATH+'api/supadmin/labopendisplay',function (data){
        console.log(data)
        if(data.code == 200){
            let Str = ''
             for (var i = 0; i < data.data.data.length; i++){
                ...
                <td class="am-text-center am-text-middle">${result.data.data[i].updated_at}</td>
                <td class="am-text-center am-text-middle">
                    ...
                    <button type="button" class="but-use">删除</button>
                </td>
            </tr>
                       `;
             }
...
function select(){
    ...
    $.ajax({
        ...
        url: SERVER_PATH + "api/supadmin/labopendisplay",
        data: {form_id: a},
        dataType: 'json',
